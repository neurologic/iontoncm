...
function batchrasteriontoFiguresSplit(start,stop,blocksize,blocktype,blockstoshow,sitetype,smooth,doprintout)
...
for i=1:length(files)
    close all
    clearvars -except blockstoshow dosave files i start stop blocksize blocktype sitetype smooth doprintout
    
    
    
    rasteriontoFiguresSplit(files(i).name,start,stop, blocksize,blocktype,blockstoshow,sitetype,smooth,doprintout,dosave)
