...
baseisis=[];
for l =1:basereps
    baseisis=[baseisis; allisis{l}];
        
end
drugisis=[];
for m =basereps+1:basereps+drugreps
    drugisis=[drugisis; allisis{m}];
        
end
washisis=[];
for n =basereps+drugreps+1:nreps
    washisis=[washisis; allisis{n}];
...
allbasego=mean(oldrate(1:basereps,gostims));
basego=mean(allbasego);
alldruggo=mean(oldrate(basereps+1:basereps+drugreps,gostims));
druggo=mean(alldruggo);
allwashgo=mean(oldrate(basereps+drugreps+1:basereps+drugreps+washreps,gostims));
washgo=mean(allwashgo);

allbasenogo=mean(oldrate(1:basereps,nogostims));
basenogo=mean(allbasenogo);
alldrugnogo=mean(oldrate(basereps+1:basereps+drugreps,nogostims));
drugnogo=mean(alldrugnogo);
allwashnogo=mean(oldrate(basereps+drugreps+1:basereps+drugreps+washreps,nogostims));
washnogo=mean(allwashnogo);
...
% basebias=(basenov)./(basenov+basetrain)
% drugbias=(drugnov)./(drugnov+drugtrain)
% washbias=(washnov)./(washnov+washtrain)
...
zbase=zscore(baserate);
zdrug=zscore(drugrate);
zwash=zscore(washrate);

zbasego=mean(zbase(gostims));
zbasenogo=mean(zbase(nogostims));
zbasenov=mean(zbase(novstims));
zbasetrain=mean(zbase(trainstims));

zdruggo=mean(zdrug(gostims));
zdrugnogo=mean(zdrug(nogostims));
zdrugnov=mean(zdrug(novstims));
zdrugtrain=mean(zdrug(trainstims));
...
if isempty(find(drugrate==0))==0;
    baserate(find(drugrate==0))=0.01;
    
end

perdiff=100*((drugrate-baserate)./(baserate));
trainperdiff=perdiff(trainstims);
novperdiff=perdiff(novstims);
