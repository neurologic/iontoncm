...
function [ax]=rasteriontoFiguresSplit(toelist, start,stop, blocksize,blocktypes,blockstoshow,sitetype, smooth, doprintout,dosave)
...
xbins = linspace(start, stop, (stop-start)*bps);    
  ...
  if find(blockstoshow==0)>0
       h=area(xbins,gzpsth-basepsth);
        set(h,'facecolor',[1 .5 .5])
      set(h,'edgecolor',[1 .5 .5])
  % plot(xbins,recpsth,'g','linewidth',1) 
  end
  if find(blockstoshow==-1)>0
       h=area(xbins,(gzpsth+1)./(basepsth+1));
       mingz=min(gzpsth)
       minbase=min(basepsth)
        set(h,'facecolor',[1 .5 .5])
     set(h,'edgecolor',[1 .5 .5])
...
if dosave==1
    outfname = sprintf('%s%s%sSplit%d',subjectID,site,stim,blockstoshow);      
